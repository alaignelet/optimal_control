...
deltax=0.025; %gridspacing
h=.05*deltax;
ncont=200;

x=1;%bounds to solve in a square [-x x][-x x]
...
while (diff>tol)
    Varr=interpn(X,Y,Vold,arrx,arry,'linear',5);
...
%SDRE solution
Pi=@(x1,x2)[((x1^4 + 1)^(1/2)*(2*(x1^4 + 1)^(1/2) + 2*x1^2 + 1)^(1/2))/2 (x1^4 + 1)^(1/2)/2 + x1^2/2;(x1^4 + 1)^(1/2)/2 + x1^2/2 (2*(x1^4 + 1)^(1/2) + 2*x1^2 + 1)^(1/2)/2];
