...
class NeuralNet(nn.Module):
    ...
    def __init__(
        ...
        **parameters
        ...
        if self.imposePsd:
            ...
            self.orthogonalMatrix = [
                torch.tensor([[-1, -1], [-1, 1]]).float() / np.sqrt(2)
            ]
    ...
    def _buildLayers(self, layers, imposePsd, spd):
        ...
        if spd:
            neuralNetLayers.append(
                spdlayers.Cholesky(
                    output_shape=2, symmetry="anisotropic", positive="Exp"
                )
            )
    ...
    def _choleskyEvaluated(self, x):
        ...
        valueFunction = 0.5 * torch.einsum("ni, nij, nj -> n", x, stackedP, x).reshape(
            -1, 1
        ).to(self.device)
    ...
    def _matrixEvaluatedPsd(self, x):
        ...
        valueFunction = 0
        for idx, Q in zip(
            [i * dim for i in range(self.countMatrices)], self.orthogonalMatrix
        ):
            ...
            valueFunction += 0.5 * torch.einsum(
                "ni, nij, nj -> n", x, stackedMatricesPsd, x
            ).reshape(-1, 1)
    ...
    def train(
        self, feedDict, lrs, iterations, useTestData, verbose=False, optimizer="adam"
    ):
        """Training function."""
        ...
        distMatrix = cdist(
            xInt.detach().numpy(),
            xInt.detach().numpy(),
            lambda u, v: np.exp(-np.linalg.norm(u - v) ** 2 / s),
        )
        if epochTotal == 0:
            print(distMatrix)
            ...
            if optimizer == "lbfgs":
                ...
                for epoch in range(iteration):
                    ...
                    def closure():
                        ...
                        loss += (
                            1e-1
                            * (
                                (yInt - yInt.T) ** 2 * torch.tensor(distMatrix).float()
                            ).sum()
                        )
                ...
                for epoch in range(iteration):
                    ...
                    if epochTotal % 100 == 0:
                        ...
                        mse = torch.tensor(0.0).to(self.device)
                        ...
                        if useTestData:
                            ...
                            lossDataTest = (
                                torch.mean(
                                    (yDataTest.double() - yTrueTest.double()) ** 2
                                )
                                .float()
                                .to(self.device)
                            )
                            print(
                                "lossDataTest: %.2e"
                                % lossDataTest.detach().cpu().numpy().item()
                            )
                            mse = lossDataTest

                            # if  gamma['gradient'] > 0.:
                            gradDataTest = self.computeValueFunctionDerivative(
                                xDataTest
                            )
                            lossGradientTest = (
                                torch.mean(
                                    (gradDataTest.double() - gradTrueTest.double()) ** 2
                                )
                                .float()
                                .to(self.device)
                            )
                            print(
                                "lossGradientTest: %.2e"
                                % lossGradientTest.detach().cpu().numpy().item()
                            )
